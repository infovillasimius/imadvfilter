...
[B1, varl]=adaptivefilter(I,n,'avg'); 
B3=adaptivefilter(I,n,'advavg',nan,4);
% B2=adaptivefilter(I,n,'advavg',noise);
% B=adaptivefilter(I,n,'avg',noise);
k=fspecial('average',n);
R=imfilter(I,k);
f1=figure('Position',[90,90,1450,700]);
...
subplot(1,2,1),imshow(B1), title({'Risultato filtro adattivo locale ' + string(num2str(n))+'x' + string(num2str(n)); secondline } ,'FontSize',9);
% subplot(1,2,2),imshow(B2), title('Filtro adattivo locale migliorato ' + string(num2str(n))+'x' + string(num2str(n)) ,'FontSize',9);
secondline='var calcolata: ' + string(num2str(varl));
subplot(1,2,2),imshow(B3), title({'Filtro adattivo locale migliorato ' + string(num2str(n))+'x' + string(num2str(n)); secondline } ,'FontSize',9);